...
def load_trello_log(ait_id, document_collection, trello_api, user_token):
    """
    ...
    documents = []
    try:
        response = requests.get(f"https://trello.com/1/boards/UklEwtTr/actions?key={trello_api}&token={user_token}")
...
def load_trello_user(ait_id, document_collection, trello_api, user_token):
    """
    ...
    documents = []
    try:
        response = requests.get(f"https://trello.com/1/members/me?key={trello_api}&token={user_token}")
        response.raise_for_status()
...
def load_trello_card(ait_id, document_collection, trello_api, user_token):
    """
    ...
    documents = []
    try:
        response = requests.get(f"https://trello.com/1/boards/UklEwtTr/cards?key={trello_api}&token={user_token}")
        response.raise_for_status()
...
def load_trello_member(ait_id, document_collection, trello_api, user_token):
    """
    ...
    documents = []
    try:
        response = requests.get(f"https://trello.com/1/boards/UklEwtTr/members?key={trello_api}&token={user_token}")
        response.raise_for_status()
...
def load_trello_documents(ait_id, logger=None):
    trello_api = os.getenv("TRELLO_API_KEY")
    user_token = os.getenv("TRELLO_USER_TOKEN")
    trello_user_documents = load_trello_user(
        ait_id=ait_id,
        document_collection="trello_user",
        trello_api=trello_api,
        user_token=user_token
    )
    trello_card_documents = load_trello_card(
        ait_id=ait_id,
        document_collection="trello_card",
        trello_api=trello_api,
        user_token=user_token
    )
    trello_log_documents = load_trello_log(
        ait_id=ait_id,
        document_collection="trello_log",
        trello_api=trello_api,
        user_token=user_token
    )
    trello_member_documents = load_trello_member(
        ait_id=ait_id,
